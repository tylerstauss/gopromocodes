...
User.create(email: 'alison.stauss@gmail.com', password: 'stauss11', username: 'alison', admin: true)
...
csv.each do |row|
  ...
  promocode = PromoCode.where(title: row[0], description: row[3], code: row[2], store_id: store.id, link: row[1]).first_or_create
  if row[9] != 'NULL'
