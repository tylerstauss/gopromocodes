...
class ApiController < ApplicationController
	...
	def coupons_test
		...
		@promo_codes = []
		@promo_codes = PromoCode.where(approved: true).where("promo_codes.expires >= '#{today}'").or(expires: nil).joins(:store).select("promo_codes.title","promo_codes.description","promo_codes.link as url","promo_codes.starts as startDate","promo_codes.expires as endDate","stores.viglink_id as merchantId","promo_codes.code as promocode").where("viglink_id is not null").limit(3000)
		p @promo_codes
		if keyword
