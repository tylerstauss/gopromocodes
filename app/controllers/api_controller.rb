...
class ApiController < ApplicationController
	...
	def coupons_test
		today = Date.today
		today = params[:enddate] if params[:enddate]
		keyword = params[:keyword].downcase if params[:keyword]
		expires_after_today = Date.today
		@promo_codes = []
		@promo_codes = PromoCode.where(approved: true).where("promo_codes.expires >= '#{today}' or promo_codes.expires is nil").joins(:store).select("promo_codes.title","promo_codes.description","promo_codes.link as url","promo_codes.starts as startDate","promo_codes.expires as endDate","stores.viglink_id as merchantId","promo_codes.code as promocode").where(approved: true).where("promo_codes.expires >= '#{expires_after_today}' and viglink_id is not null").limit(3000)
		if keyword
